...
function evaluate(expr, props) {
  ...
  if (m = t.match(/^(.+?)\s*(===|==|!==|!=|>=|<=|>|<)\s*(.+)$/)) {
    const [, l, op, r] = m, lv = evaluate(l, props), rv = evaluate(r, props);
    return { '===': lv === rv, '==': lv == rv, '!==': lv !== rv, '!=': lv != rv,
             '>': lv > rv, '<': lv < rv, '>=': lv >= rv, '<=': lv <= rv }[op];
  }
  ...
  if (m = t.match(/^(.+?)\s*([\+\-])\s*(.+)$/)) {
    ...
    if (m[2] === '+') return String(lv ?? '') + String(rv ?? '');
  }

  if (m = t.match(/^(.+?)\.(\w+)\(([^)]*)\)(\[\d+\])?$/)) {
    const obj = evaluate(m[1], props);
    if (!obj || !safeMethods.has(m[2])) return;
    const arg = m[3].match(/^['"](.*)['"]$/) ? m[3].slice(1, -1)
              : /^-?\d+$/.test(m[3]) ? +m[3] : undefined;
  ...
  if (t === 'null' || t === 'undefined') return t === 'null' ? null : undefined;
    ...
    return items.map((item, i) => {
      let o = block
        .replace(/\{\{#if\s+([^}]+)\}\}([^{]*?)(?:\{\{else\}\}([^{]*?))?\{\{\/if\}\}/g,
          (_, c, y, n = '') => evaluate(c, { ...props, ...item }) ? y : n)
