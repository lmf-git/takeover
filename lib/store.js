...
const getTheme = () => isBrowser ? localStorage.getItem('theme') ?? (matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light') : 'light';
...
Object.assign(store, {
  toggleTheme() {
    ...
    if (isBrowser) { localStorage.setItem('theme', theme); document.documentElement.dataset.theme = theme; }
  },
  login(creds = {}) {
    const user = {
...
if (isBrowser) document.documentElement.dataset.theme = getTheme();
