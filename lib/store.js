...
// Global singleton so the IIFE bundle and dynamically-imported native ES modules
// share one store instance — without this each module load creates its own store
// with empty messages, breaking translations on client-side navigation.
const store = isBrowser
  ? (window.__store__ ??= new Store({ ...defaults, theme: getTheme(), user: getUser(), isAuthenticated: !!getUser() }, defaults))
