...
/**
 * Smooth-scroll to an in-page anchor, descending through shadow roots.
 * Each section lives inside its component's shadow DOM, so the browser's
 * native #hash navigation can't find the target — we walk the composed tree.
 * @param {string} hash - Target id, with or without a leading '#'
 * @returns {boolean} - true if a matching element was found and scrolled to
 */
export function scrollToHash(hash) {
  ...
  if (!id) return false;
  const find = root => {
    const direct = root.getElementById ? root.getElementById(id) : root.querySelector(`[id="${CSS.escape(id)}"]`);
...
export default { navigate, replace, back, forward, redirectToLogin, scrollToHash, getQuery, getQueryParam, setQuery, buildUrl, isActive, parseParams };
